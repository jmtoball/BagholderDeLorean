...
- **`api`** (`bagholder-api`) — axum server. Holds the `Store` behind `Arc<Mutex<>>` (one global lock — fine for single-user dev). Endpoints: `GET /api/backtest` (load cached data + run engine; `entry=pe_min` enters at the most recent local-minimum P/E instead of a fixed `years` window — uses `core::pe_series` + `core::local_minima`, returns the chosen `entry_date`/`entry_pe`), `GET /api/fundamentals?ticker=`, `GET /api/screen?kind=low_pe&limit=N` (rank `DEFAULT_UNIVERSE` by industry-relative P/E — **cold call warms ~23 names, ~2 min; cached after**). Also serves `crates/web/dist`. Blocking DB/network calls go through `tokio::task::spawn_blocking` — keep them off the async runtime's worker threads.
